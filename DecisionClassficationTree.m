function [tree] = DecisionClassficationTree(x,y,depth, attribute_name)
    ...
    if best_gain < min_gain || isempty(left) || isempty(right)
        ...
        if length(unique(y_left)) == 1
            ...
            tree.kids{1} = DecisionClassficationTree(x_left, y_left, depth, attribute_name);
        end
        if length(unique(y_right)) == 1
            ...
            tree.kids{2} = DecisionClassficationTree(x_right, y_right, depth, attribute_name);
        end
        depth = depth-1;
    end
end
